...
function craftory_content_nav( $nav_id ) {
	...
	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
		...
		<?php 
		if ( is_search() ) {
			$next = __( '<span class="meta-nav">&larr;</span> More results', 'craftory' );
			$previous = __( 'Previous results <span class="meta-nav">&rarr;</span>', 'craftory' );
		}
		else {
			$next = __( '<span class="meta-nav">&larr;</span> Older posts', 'craftory' );
			$previous = __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'craftory' );
		} 
		?>
